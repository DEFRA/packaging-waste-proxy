# Architecture and follow-up review

This document records the evidence and design decisions retained by `packaging-waste-proxy`, including its implementation and testing patterns.

## Summary

This CDP-hosted YARP reverse proxy exposes a logical downstream service beneath a public
...
flowchart LR
    ...
    reportProxy["packaging-waste-proxy\nYARP, HTTPS on :8015"]
...
`packaging-waste-proxy`. The proxy removes the public path prefix and calls the internal
...
[`cdp-app-config`](https://github.com/DEFRA/cdp-app-config/tree/main/services/packaging-waste-proxy), rather
...
Use `packaging-waste-proxy` as the CDP proxy service for the Packaging Waste logical group. It uses the established bootstrap, configuration-validation, routing, logging, trace-correlation, unit-test, and integration-test patterns. This keeps the public routing surface deliberately small and explicit, while allowing the proxy to be deployed, configured, monitored, and supported independently.
